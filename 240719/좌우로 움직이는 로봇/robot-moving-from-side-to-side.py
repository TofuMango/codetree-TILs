...
if A_time > B_time:
    maxtime = A_time
    for i in range(B_time, maxtime+1):
        pos_B[B_time] = pos_B[B_time-1]
        B_time +=1
else:
    maxtime = B_time
    for i in range(A_time, maxtime+1):
        pos_A[A_time] = pos_A[A_time-1]
        A_time +=1

ans = 0
for i in range(1, max(A_time, B_time)):
    # 현재시간의 이동거리가 일치하면서, 이전시간의 거리는 일치하지않을때만 +1
